...
ASNTagLookup = ["reserved for BER",
"BOOLEAN", "INTEGER", "BIT STRING", "OCTET STRING", "NULL", "OBJECT IDENTIFIER",
"ObjectDescriptor", "INSTANCE OF, EXTERNAL", "REAL", "ENUMERATED", 
"EMBEDDED PDV","UTF8String", "RELATIVE-OID", "undefined ", "undefined ", "SEQUENCE, SEQUENCE OF", "SET, SET OF", "NumericString", "PrintableString", "TeletexString, T61String","VideotexString", "IA5String", "UTCTime", "GeneralizedTime", "GraphicString", "VisibleString, ISO646String", "GeneralString", "UniversalString", "CHARACTER STRING", "BMPString"]    
...
class Encoder(object):
    ...
    def enter(self, nr, cls=None):  # type: (int, int) -> None
        ...
        if self.m_stack is None:
            logging.error('Encoder not initialized. Call start() first.')
    ...
    def leave(self):  # type: () -> None
        ...
        if len(self.m_stack) == 1:
            logging.error('Tag stack is empty. Nothing to do.')
...
class Decoder(object):
    ...
    def start(self, data):  # type: (bytes) -> None
        ...
        if not isinstance(data, bytes):
            logging.error('Expecting bytes instance.')
    ...
    def read(self, tagnr=None):  # type: (Number) -> (Tag, any)
        ...
        logging.info('read {} {}'.format(tag, length))
    ...
    def enter(self):  # type: () -> None
        ...
        length = self._read_length()
        print(' enter - {}: len {}'.format(tag,length))
    ...
    def _read_tag(self):  # type: () -> Tag
        ...
        if nr == 0x1f:  # Long form of tag encoding
            ...
            while True:
                byte = self._read_byte()
                nr = (nr << 7) | (byte & 0x7f)
                logging.info(' long tag byte {} nr {} '.format(hex(byte), hex(nr)))
                if not byte & 0x80:
                    break
        if typ == Types.Primitive:
            logging.info('read_tag {} {} {} Primative {}'.format(hex(byte), cls, typ, ASNTagLookup[nr]))
        else:
            logging.info('read_tag {} {} {} Constructed {}'.format(hex(byte), cls, typ, nr))
    ...
    def _read_length(self):  # type: () -> int
        ...
        if byte & 0x80:
            ...
            try:
                ...
                pass
            logging.info(' long_len of {} bytes 0x{}'.format(length, bytes_data.hex()))
        else:
            length = byte
            logging.info('read_len {} byte {}'.format(length, hex(byte)))
        return length
    ...
    def _read_bytes(self, count):  # type: (int) -> bytes
        ...
        if len(bytes_data) != count:
            raise Error('Premature end of input. len(bytes_data){} count{}'.format(len(bytes_data), count))
    ...
    def _decode_boolean(bytes_data):  # type: (bytes) -> bool
        ...
        if len(bytes_data) != 1:
            #raise Error
            logging.error('ASN1 syntax error - expected boolean but length is {}'.format(
                len(bytes_data)))
        if bytes_data[0] == 0:
            return False
        return True
